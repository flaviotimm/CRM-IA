...
export default function AgenteIA() {
  ...
  // Ref sempre com a config mais recente, para o flush no unmount conseguir
  // ler o valor atual sem precisar recriar o efeito de unmount a cada tecla.
  const cfgRef = useRef<SdrConfig | null>(null);
  cfgRef.current = cfg;

  // Auto-save com debounce: evita perder edições ao trocar de aba antes de
  // clicar em "Salvar agente" (o estado de `cfg` só existe neste componente
  // e é descartado quando a tela é desmontada).
  useEffect(() => {
    if (!cfg) return;
    ...
    autosaveTimer.current = setTimeout(() => {
      backend.saveSdrConfig(cfg);
      autosaveTimer.current = null;
    }, 800);
  }, [cfg]);

  // Ao sair da tela (troca de aba/navegação), salva imediatamente qualquer
  // edição pendente que o debounce ainda não tenha disparado.
  useEffect(() => {
    return () => {
      if (autosaveTimer.current) {
        clearTimeout(autosaveTimer.current);
        if (cfgRef.current) backend.saveSdrConfig(cfgRef.current);
      }
    };
  }, []);
